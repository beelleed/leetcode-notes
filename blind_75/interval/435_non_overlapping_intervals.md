...
### 如果用開始排列intervals.sort(key=lambda x:x[0])
- 範例: intervals = [[1,100], [11,12], [12,13], [13,14]]
...
- 你的程式：
    ...
    - 看 [11,12]：start=11 < 100 → count=1（你等於刪掉 [11,12]）

    - 看 [12,13]：12 < 100 → count=2

    - 看 [13,14]：13 < 100 → count=3

- 最後回傳 3。

- 但直覺就知道：其實只要移除那個超大的 [1,100]，後面三個幾乎都能留下來，移除數不會那麼多。

👉 錯誤來源就是：重疊時你一直死守 prev_end=100 不放。
