...
| 區段      | 程式碼                                                                                 | 功能 / 解釋                           |
| ------- | ----------------------------------------------------------------------------------- | --------------------------------- |
| 檢查空字串   | `if not s: return ""`                                                               | 若輸入是空字串，直接回傳空字串作為結果               |
| 初始化回文區間 | `start, end = 0, 0`                                                                 | 用來記錄目前找到的 **最長回文子串** 的開始與結束索引     |
| 定義擴展函式  | `def expand(left: int, right: int) -> (int, int): …`                                | 從中心往左右擴展，檢查是否仍為回文，最後回傳擴展後的合法區間    |
| 擴展邏輯    | `while left >= 0 and right < len(s) and s[left] == s[right]: left -= 1; right += 1` | 當左右字符相等且還在邊界之內，就繼續擴展；否則停止         |
| 回傳合法邊界  | `return left + 1, right - 1`                                                        | 擴展停止時，左右指標已經越界或不匹配，所以回退一步得到合法回文區間 |
| 主迴圈遍歷中心 | `for i in range(len(s)):`                                                           | 把每一個位置 `i` 當作回文中心（奇數與偶數兩種可能）      |
| 奇數回文中心  | `l1, r1 = expand(i, i)`                                                             | 中心在同一字符（以字符本身為中心擴展）               |
| 偶數回文中心  | `l2, r2 = expand(i, i + 1)`                                                         | 中心在兩字符之間（適用於偶數長度回文）               |
| 更新最長    | `if r1 - l1 > end - start: start, end = l1, r1` 和類似的 for r2/l2                      | 若新找到的回文比目前記錄的長，就更新 `start, end`   |
| 回傳子字串   | `return s[start:end + 1]`                                                           | 回傳最終最大回文子串（包含 `end` 索引）           |
