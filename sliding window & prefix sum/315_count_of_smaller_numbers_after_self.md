...
│   └── 合併 [5] 和 [2]
│       ├── 比較 nums[1]=2 < nums[0]=5 → 是
│       │   └── right_counter += 1 → right_counter = 1
│       └── 將 nums[0]=5 加入 temp，result[0] += right_counter → result[0] = 1
│       合併後 index: [1, 0]
├── 右半部: [6, 1] (index: [2, 3])
│   ├── 分割為 [6] (index: [2]) 和 [1] (index: [3])
│   └── 合併 [6] 和 [1]
│       ├── 比較 nums[3]=1 < nums[2]=6 → 是
│       │   └── right_counter += 1 → right_counter = 1
│       └── 將 nums[2]=6 加入 temp，result[2] += right_counter → result[2] = 1
│       合併後 index: [3, 2]
第二步：合併左右半部
├── 合併 [1, 0] 和 [3, 2]
│   ├── 比較 nums[3]=1 < nums[1]=2 → 是
│   │   └── right_counter += 1 → right_counter = 1
│   ├── 比較 nums[2]=6 < nums[1]=2 → 否
│   │   └── 將 nums[1]=2 加入 temp，result[1] += right_counter → result[1] = 1
│   ├── 比較 nums[2]=6 < nums[0]=5 → 否
│   │   └── 將 nums[0]=5 加入 temp，result[0] += right_counter → result[0] = 2
│   └── 將剩餘的 nums[2]=6 加入 temp
│   合併後 index: [3, 1, 0, 2]
最終 result: [2, 1, 1, 0]
```
📌 說明
